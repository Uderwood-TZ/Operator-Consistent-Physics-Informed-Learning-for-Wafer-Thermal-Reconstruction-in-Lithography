...
class PINN_LSTM(torch.nn.Module):
    def __init__(self):
        super().__init__()
        self.input_layer = torch.nn.Linear(2, 64)
        self.act = torch.nn.Tanh()


        self.lstm1 = torch.nn.LSTM(input_size=64, hidden_size=64, num_layers=1, batch_first=True)
        self.lstm2 = torch.nn.LSTM(input_size=64, hidden_size=64, num_layers=1, batch_first=True)
