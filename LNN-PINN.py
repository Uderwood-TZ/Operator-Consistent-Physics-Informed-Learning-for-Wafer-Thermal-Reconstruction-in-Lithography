...
class LiquidNeuron(torch.nn.Module):
    def __init__(self, size):
        ...
        self.alpha = torch.nn.Parameter(torch.ones(size) * 0.5)

    def forward(self, h):
        dh = torch.tanh(self.W(h))
        return h + self.alpha * dh


class LiquidNet(torch.nn.Module):
    def __init__(self):
        super().__init__()
        self.input = torch.nn.Linear(2, 64)
        self.liquid1 = LiquidNeuron(64)
        self.linear1 = torch.nn.Linear(64, 64)
        self.liquid2 = LiquidNeuron(64)
        self.linear2 = torch.nn.Linear(64, 64)
        self.liquid3 = LiquidNeuron(64)
